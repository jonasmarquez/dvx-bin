...
```bash
curl -L -o dvx_0.12.0_darwin_arm64.tar.gz   https://github.com/jonasmarquez/dvx-bin/releases/download/v0.12.0/dvx_0.12.0_darwin_arm64.tar.gz

tar xzf dvx_0.12.0_darwin_arm64.tar.gz
chmod +x dvx
...
Export only K8s-related variables:

```bash
eval "$(dvx k8s env)"
# Now KUBECONFIG, KUBE_CONTEXT and KUBE_NAMESPACE are set (if defined)
```
...
This sets the selected dimension as current, and then you can use
`dvx k8s env`, `dvx k8s shell`, etc.
